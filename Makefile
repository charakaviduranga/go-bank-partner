.PHONY: help build run test test-coverage clean docs swagger postman install-tools mocks lint migrate-up migrate-down seed migrate
...
migrate:
	...
	migrate create -ext sql -dir db/migrations -seq $(word 2,$(MAKECMDGOALS))

# swallow extra args so make doesn’t try to build them
%:
	@:

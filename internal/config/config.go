...
type RateLimiterConfig struct {
	...
	RateLimitRequestPerSeconds int
	RateLimitBurst             int
}
...
func Load() *Config {
	...
	config := &Config{
		...
		NorthWind: NorthWindConfig{
			BaseURL:    getEnv("NORTHWIND_BASE_URL", "https://api.northwind-bank.com"),
			APIKey:     getEnv("NORTHWIND_API_KEY", ""),
			Timeout:    getDurationEnv("NORTHWIND_TIMEOUT", 30*time.Second),
			WebhookURL: getEnv("REGULATOR_WEBHOOK_URL", "https://regulator.example.com/webhook"),
		},
		Redis: RedisConfig{
		...
		ConcurrentLimiter: ConcurrentLimiter{
			UserConcurrentLimiter: getIntEnv("USER_CONCURRENT_LIMITER", 5),
		},
	}

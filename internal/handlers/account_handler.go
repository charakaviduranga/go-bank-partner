...
type AccountHandler struct {
	...
	concurrentTransfers services.UserConcurrentLimiterInterface
	northWindService    services.NorthWindServiceInterface
}

// NewAccountHandler creates a new account handler
func NewAccountHandler(accountService services.AccountServiceInterface, auditLogger services.AuditLoggerInterface, metricsCollector services.MetricsRecorderInterface, concurrentTransfers services.UserConcurrentLimiterInterface, northWindService services.NorthWindServiceInterface) *AccountHandler {
...
func (h *AccountHandler) CreateAccount(c echo.Context) error {
	...
	accountBalance, currency, err := h.northWindService.GetAccountBalance(c.Request().Context(), req.AccountNumber)

	if accountBalance.LessThan(req.InitialDeposit) {
		return SendError(c, errors.TransactionInsufficientFunds)
	}

	account, err := h.accountService.CreateAccount(userID, req.AccountType, req.AccountNumber, req.RoutingNumber, currency, req.InitialDeposit)
	if err != nil {
...
func (h *AccountHandler) Transfer(c echo.Context) error {
	...
	if h.concurrentTransfers.Increment(userID) > 3 {
		h.concurrentTransfers.Decrement(userID)
		return SendError(c, errors.SystemTooManyRequests)
	}
	defer h.concurrentTransfers.Decrement(userID)
	...
	transfer, err := h.accountService.TransferBetweenAccounts(ctx, dto.TransferBetweenAccountsInput{

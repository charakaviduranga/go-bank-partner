...
type AccountHandlerSuite struct {
	...
	userLimiter      services.UserConcurrentLimiterInterface
}

// simpleLimiter is a minimal test implementation of UserConcurrentLimiterInterface
type simpleLimiter struct{}

func (l *simpleLimiter) Increment(userID uuid.UUID) int { return 1 }
func (l *simpleLimiter) Decrement(userID uuid.UUID)     {}

// MockNorthWindServiceHandler is a minimal test implementation of NorthWindServiceInterface
type MockNorthWindServiceHandler struct{}

func (m *MockNorthWindServiceHandler) ValidateAccount(ctx context.Context, req dto.AccountValidationRequest) (*dto.AccountValidationResult, error) {
	return &dto.AccountValidationResult{AccountExists: true, AccountValid: true}, nil
}

func (m *MockNorthWindServiceHandler) InitiateTransfer(ctx context.Context, req dto.TransferInitiationRequest) (*dto.TransferStatus, error) {
	return &dto.TransferStatus{TransferID: uuid.New().String(), Status: "pending"}, nil
}

func (m *MockNorthWindServiceHandler) GetTransferStatus(ctx context.Context, transferID string) (*dto.TransferStatus, error) {
	return &dto.TransferStatus{TransferID: transferID, Status: "completed"}, nil
}

func (m *MockNorthWindServiceHandler) ValidateTransfer(ctx context.Context, in dto.TransferValidationInput) (*dto.ValidationResponse[dto.TransferStatus], error) {
	return &dto.ValidationResponse[dto.TransferStatus]{Validation: dto.Validation{Valid: true}}, nil
}

func (m *MockNorthWindServiceHandler) Notify(ctx context.Context, payload map[string]interface{}) error {
	return nil
}

func (m *MockNorthWindServiceHandler) GetAccountBalance(ctx context.Context, accountNumber string) (decimal.Decimal, string, error) {
	return decimal.NewFromFloat(10000.00), "USD", nil
}
...
func (s *AccountHandlerSuite) SetupTest() {
	...
	s.userLimiter = &simpleLimiter{}
	s.handler = NewAccountHandler(s.mockService, s.auditLogger, s.metricsCollector, s.userLimiter, &MockNorthWindServiceHandler{})
...
func (s *AccountHandlerSuite) TestCreateAccount_WithInitialDeposit() {
	...
	reqBody := dto.CreateAccountRequest{
		...
		InitialDeposit:    decimal.NewFromFloat(100.00),
	}
	...
	s.mockService.EXPECT().
		CreateAccount(s.testUserID, "CHECKING", gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).
		DoAndReturn(func(_ uuid.UUID, _ string, _ string, _ string, _ string, amount decimal.Decimal) (*models.Account, error) {
...
func (s *AccountHandlerSuite) TestTransfer_Success() {
	...
	// Expect service call
	s.mockService.EXPECT().
		TransferBetweenAccounts(gomock.Any(), gomock.Any()).
		DoAndReturn(func(_ context.Context, in dto.TransferBetweenAccountsInput) (*models.Transfer, error) {
			if !in.Amount.Equal(decimal.NewFromFloat(100.00)) {
				s.T().Errorf("expected amount 100.00, got %s", in.Amount.String())
			}
			debitTxID := uuid.New()
			creditTxID := uuid.New()
...
func (s *AccountHandlerSuite) TestTransfer_WithIdempotencyKey_Success() {
	...
	s.mockService.EXPECT().
		TransferBetweenAccounts(gomock.Any(), gomock.Any()).
		DoAndReturn(func(_ context.Context, in dto.TransferBetweenAccountsInput) (*models.Transfer, error) {
			if !in.Amount.Equal(decimal.NewFromFloat(150.00)) {
				s.T().Errorf("expected amount 150.00, got %s", in.Amount.String())
			}

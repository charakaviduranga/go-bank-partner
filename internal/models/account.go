...
func (a *Account) Validate() error {
	...
	err := a.ValidateAccountTypeMatch()

	if err != nil {
		return fmt.Errorf("account number prefix does not match account type: %w", err)
	}
...
func ValidateAccountNumber(accountNumber string) bool {
	// exact format: WVMZ28-SV-002
	pattern := `^[A-Z]{4}\d{2}-(SV|CH|MM)-\d{3}$`
	re := regexp.MustCompile(pattern)

	return re.MatchString(accountNumber)
}

func (a *Account) ValidateAccountTypeMatch() error {
	parts := strings.Split(a.AccountNumber, "-")
	if len(parts) != 3 {
		return fmt.Errorf("invalid account number format")
	}

	accountTypeSegment := parts[1] // SV, CH, MM

	expectedPrefix := GetAccountPrefix(strings.ToUpper(a.AccountType))

	if accountTypeSegment != expectedPrefix {
		return fmt.Errorf("account number prefix does not match account type %s, expected %s", accountTypeSegment, GetAccountPrefix(a.AccountType))
	}

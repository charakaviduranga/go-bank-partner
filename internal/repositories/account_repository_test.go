...
func (s *AccountRepositorySuite) TestCreate_DuplicateAccountNumber() {
	...
	account2 := &models.Account{
		UserID:        s.testUser.ID,
		AccountNumber: "ABCD12-CH-001",            // Same account number
...
func (s *AccountRepositorySuite) TestGetByAccountNumber() {
	...
	// Test getting existing account
	found, err := s.repo.GetByAccountNumber("ABCD12-CH-001")
	s.NoError(err)
	s.NotNil(found)
	s.Equal(account.AccountNumber, found.AccountNumber)
...
func (s *AccountRepositorySuite) TestGetByUserID() {
	...
	s.Contains(accountNumbers, "ABCD12-CH-001")
	s.Contains(accountNumbers, "ABCD12-SV-002")
}
...
func (s *AccountRepositorySuite) TestGenerateUniqueAccountNumber() {
	...
	s.True(len(accountNumber1) > 0)
	// Generated format includes account type prefix
	s.True(strings.HasPrefix(accountNumber1, "CH"))
	...
	s.True(strings.HasPrefix(accountNumber2, "SV"))
	...
	s.True(strings.HasPrefix(accountNumber3, "MM"))

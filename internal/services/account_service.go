...
type accountService struct {
	...
	locker                 RedisLockerInterface
	notifyRegulatorSerivce NotifyRegulatorServiceInterface
	logger                 *slog.Logger
}
...
func NewAccountService(
	...
	northWindService NorthWindServiceInterface,
	locker RedisLockerInterface,
	notifyRegulator NotifyRegulatorServiceInterface,
	...
	return &accountService{
		...
		notifyRegulatorSerivce: notifyRegulator,
		locker:                 locker,
	}
}

// CreateAccount creates a new account for a user
func (s *accountService) CreateAccount(userID uuid.UUID, accountType, accountNumber, routingNumber, currency string, initialDeposit decimal.Decimal) (*models.Account, error) {
	...
	// If accountNumber isn't provided, generate one
	if accountNumber == "" {
		accNum, err := s.accountRepo.GenerateUniqueAccountNumber(accountType)
		if err != nil {
	...
	if err := s.accountRepo.Create(newAccount); err != nil {
		s.logger.Error("Failed to create new account", "error", err)
	...
	if err := s.auditRepo.Create(&models.AuditLog{
		...
		Resource:   "account",
		ResourceID: newAccount.ID.String(),
		IPAddress:  "system",
		UserAgent:  "internal",
		...
		s.logger.Error("Failed to Create Audit Log", "error", err, "Action", "New Account Created")
	}
...
func (s *accountService) TransferBetweenAccounts(
	ctx context.Context,
	in dto.TransferBetweenAccountsInput,
) (*models.Transfer, error) {
	lockKey := fmt.Sprintf("account:%s", in.FromAccountID)
	...
	// ---- basic validation ----
	if err := s.validateTransferRequest(
		in.FromAccountID,
		in.ToAccountID,
		in.Amount,
		in.IdempotencyKey,
	); err != nil {
		s.logger.Error("Basic Validate Transfer Failed", "error", err)
		return nil, err
	}

	s.logger.Info("Basic Validate Transfer Success", "fromAccount", in.FromAccountID, "toAccount", in.ToAccountID)

	// ---- idempotency ----
	if existing, err := s.checkExistingTransfer(in.IdempotencyKey); err != nil || existing != nil {
		s.logger.Error("Transfer Already Exist")
		return existing, err
	}

	// ---- load + authorize accounts ----
	fromAccount, toAccount, err := s.retrieveAndAuthorizeAccounts(
		in.FromAccountID,
		in.ToAccountID,
		in.UserID,
	)
	...
	if err != nil {
		s.logger.Error("Northwind transfer validation failed", "error", err)
		return nil, err
	}

	if !validationRes.Validation.Valid {
		s.logger.Error("Northwind transfer validation failed", "error", err)
		return nil, errors.New("Northwind transfer validation failed")
	}
	...
	if err != nil {
		s.logger.Error("Failed Transfer Initiated", "error", err)
		return nil, err
	}

	transfer, debitTxID, creditTxID, err := s.executeTransfer(
		in.Amount, in.Description, in.IdempotencyKey,
		fromAccount, toAccount, initiated.ReferenceNumber, strings.ToLower(initiated.Status), initiated.TransferID, initiated.TransferType,
	)
	if err != nil {
		s.logger.Error("Failed Execute Transfer", "error", err)
		if transfer != nil {
			s.handleTransferFailure(transfer, err, fromAccount, toAccount, in.Amount, in.IdempotencyKey, in.UserID, initiated.Status)
		}
		return nil, err
	}

	if err := s.handleTransferSuccess(transfer, debitTxID, creditTxID, fromAccount, toAccount, in.Amount, in.IdempotencyKey, in.UserID); err != nil {
		return nil, err
	}

	s.notifyRegulatorSerivce.StartMonitoring(transfer.ID, fromAccount.AccountNumber, toAccount.AccountNumber, in.Amount.String())
...
func (s *accountService) initiateTransfer(
	...
	description, idempotencyKey, direction, institutionName string,
	...
	initiateReq := dto.TransferInitiationRequest{
		Amount:             amountFloat,
		Currency:           currency,
		Description:        description,

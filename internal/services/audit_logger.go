...
func (al *AuditLogger) LogEvent(ctx context.Context, message string, details map[string]interface{}) {
	attrs := []slog.Attr{
		slog.String("event_type", "generic_event"),
		slog.Time("timestamp", time.Now()),
		slog.String("correlation_id", getCorrelationID(ctx)),
	}
	...
	al.logger.LogAttrs(ctx, slog.LevelInfo, message, attrs...)
}

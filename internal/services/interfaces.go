...
type AccountServiceInterface interface {
	CreateAccount(userID uuid.UUID, accountType, accountNumber, routingNumber, currency string, initialDeposit decimal.Decimal) (*models.Account, error)
	...
	TransferBetweenAccounts(ctx context.Context, in dto.TransferBetweenAccountsInput) (*models.Transfer, error)
...
type AuditLoggerInterface interface {
	...
	LogEvent(ctx context.Context, message string, details map[string]interface{})
}
...
type NorthWindServiceInterface interface {
	...
	Notify(ctx context.Context, paylod map[string]interface{}) error
	GetAccountBalance(ctx context.Context, accountNumber string) (decimal.Decimal, string, error)
}

type RedisLockerInterface interface {
	Lock(ctx context.Context, key string, ttl time.Duration) error
	Unlock(ctx context.Context, key string) error
}

type UserConcurrentLimiterInterface interface {
	Increment(userID uuid.UUID) int
	Decrement(userID uuid.UUID)
}

type NotifyRegulatorServiceInterface interface {
	StartMonitoring(transferID uuid.UUID, fromAccount, toAccount, amount string)
	PollTransferStatus(ctx context.Context, transferID uuid.UUID) string
	NotifyRegulator(ctx context.Context, transferID uuid.UUID, success bool, details map[string]interface{})
}
